...
(function () {
  ...
  function runWhenReady() {
    ...
    function getCourseAttachedEbookIds(courseDoc) {
      if (!courseDoc || typeof courseDoc !== "object") return [];
      if (
        Array.isArray(courseDoc.attachedCourseEbookIds) &&
        courseDoc.attachedCourseEbookIds.length
      ) {
        return courseDoc.attachedCourseEbookIds;
      }
      // 레거시 호환: 예전 attachedEbookIds 를 읽어 기존 강의 수정 시 체크 유지
      if (Array.isArray(courseDoc.attachedEbookIds) && courseDoc.attachedEbookIds.length) {
        return courseDoc.attachedEbookIds;
      }
      return [];
    }
    ...
    function renderAttachedEbookPicker(selectedIds) {
      ...
      return db
        .collection(COURSE_EBOOKS_COLLECTION)
        .get()
        .then(function (snap) {
          if (snap.empty) {
            mount.innerHTML =
              '<p class="text-muted small mb-0">등록된 강의 전용 전자책이 없습니다. 아래 업로드 행에서 파일을 추가해 주세요.</p>';
            return;
          }
          var rows = [];
     ...
     * @returns {Promise<string[]>} 새 course_ebooks 문서 id 목록
     */
      ...
      function uploadOne(item) {
        ...
        return uploadFile(item.file, path).then(function (url) {
          var authorName = item.authorName || courseInstructor || "강의";
            return db.collection(COURSE_EBOOKS_COLLECTION).get().then(function (snap) {
            return db.collection(COURSE_EBOOKS_COLLECTION).add({
              title: item.title,
    ...
    function loadCourseForEdit() {
      ...
      return db
        ...
        .then(function (doc) {
          ...
          var ae = getCourseAttachedEbookIds(currentCourseDoc);
          ...
          return uploadNewEbooksCreateDocs(courseInstructor).then(function (newEbookIds) {
            return buildPartsArray(refId, rawPayload).then(function (partsOut) {
              ...
              var data = {
                ...
                attachedCourseEbookIds: mergedEbookIds,
    ...
    if (btnCancel) {
      btnCancel.addEventListener("click", function () {
        ...
        var restoreIds =
          editCourseId && currentCourseDoc ? getCourseAttachedEbookIds(currentCourseDoc) : [];
