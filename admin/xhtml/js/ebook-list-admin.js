/**
 * 관리자 전자책 목록: Firestore ebooks 로드, 표시 순서(order) 변경, 수정·삭제
 */
(function () {
  var firebaseConfig = {
  ...
  function run() {
    ...
    function getOrderNum(d) {
      var v = d && d.order;
      if (typeof v === "number" && !isNaN(v)) return v;
      return null;
    }

    function sortEbookDocs(docs) {
      return docs.slice().sort(function (a, b) {
        var oa = getOrderNum(a.data());
        var ob = getOrderNum(b.data());
    ...
    function renumberAllOrders(sortedDocs) {
      var batch = db.batch();
    ...
    function swapAdjacentOrders(sortedDocs, lowerIndex) {
      ...
      return renumberAllOrders(reordered);
    }
        ...
        .then(function (snap) {
          if (snap.empty) {
            setTbody(
              '<tr><td colspan="' +
                COL_SPAN +
                '" class="text-center text-muted py-4">등록된 전자책이 없습니다.</td></tr>'
            );
            return;
          }
          var sorted = sortEbookDocs(snap.docs);
          var html = "";
          sorted.forEach(function (doc, rowIndex) {
            ...
            html +=
              ...
              '<button type="button" class="btn btn-sm btn-outline-secondary btn-ebook-order-up me-1"' +
              upDisabled +
              ' data-row="' +
              rowIndex +
              '" title="위로">↑</button>' +
              '<button type="button" class="btn btn-sm btn-outline-secondary btn-ebook-order-down"' +
              downDisabled +
              ' data-row="' +
              rowIndex +
              '" title="아래로">↓</button>' +
              ...
              "</td>" +
              '<td class="text-end">' +
              '<a href="ebook-upload.html?edit=' +
              escapeAttr(id) +
              '" class="btn btn-sm btn-outline-primary me-1">수정</a>' +
              '<button type="button" class="btn btn-sm btn-outline-danger" data-ebook-id="' +
              escapeAttr(id) +
              '" data-ebook-title="' +
              escapeAttr(title) +
              '">삭제</button>' +
          ...
          tbody.querySelectorAll(".btn-ebook-order-up").forEach(function (btn) {
          ...
          tbody.querySelectorAll(".btn-ebook-order-down").forEach(function (btn) {
            btn.addEventListener("click", function () {
              var row = parseInt(btn.getAttribute("data-row"), 10);
